...
barras <-
  tibble(Barra = c("Paranaguá", "Ararapira", "Cananéia", "Icapara"),
         lng = c(-48.286840, -48.054815, -47.896897, -47.418348),
         lat = c(-25.511408, -25.265283, -25.059377, -24.674825)) %>%
...
tab_3 <- 
  ...
  arrange(Rota) %>%
  select(2:3) %>% 
  addHtmlTableStyle(align = "c|c", 
                    css.cell = c("width: 100;","width: 100;")) %>%
  htmlTable(rnames = FALSE,
            caption = "Objetivo 3")
...
pal_blue <- colorNumeric(palette = "Blues",
                         domain = 3:-3)
...
leaflet() %>%
  ...
  addPolygons(data = area_1, group = "Área de Estudo 1", weight = 2, color = ~pal_blue(Rota)) %>%
  addPolygons(data = area_2, group = "Área de Estudo 2", weight = 2, color = ~pal_blue(Rota)) %>%
  addPolygons(data = area_3, group = "Área de Estudo 3", weight = 2, color = ~pal_blue(Rota)) %>%
  addPolylines(data = rota_1, group = "Área de Estudo 1", weight = 2, color = ~pal_blue(Rota)) %>%
  addPolylines(data = rota_2, group = "Área de Estudo 2", weight = 2, color = ~pal_blue(Rota)) %>%
  addCircleMarkers(data = pnts, group = "Área de Estudo 3", color = "blue", radius = 4) %>%
  addPolylines(data = rota_pas, group = "Área de Estudo 2", color = ~pal_lines(Tipo)) %>%
  addPolylines(data = rota_bal, group = "Área de Estudo 2", color = ~pal_lines(Tipo)) %>%
  addPolylines(data = rota_pes, group = "Área de Estudo 2", color = ~pal_lines(Tipo)) %>%
  addPopups(-47.891029, -24.910342, tab_1, group = "Medidas 1",
            options = popupOptions(closeButton = FALSE)) %>%
  addPopups(-48.022023, -25.027921, tab_2, group = "Medidas 2",
            options = popupOptions(closeButton = FALSE)) %>%
  addPopups(-47.949324, -25.061980, tab_3, group = "Medidas 3",
            options = popupOptions(closeButton = FALSE)) %>%
  addCircleMarkers(data = barras, group = "Barras", color = "orange", radius = 7) %>%
  addLayersControl(baseGroups = c("Satélite", "Mapa"),
                   overlayGroups = c(
                     "Barras",
                     "Área de Estudo 1",
                     "Medidas 1",
